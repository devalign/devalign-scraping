...
class ArbeitnowParser(BaseParser):
    ...
    def fetch_and_parse_job(self, page, url: str) -> JobOffer:
        ...
        if created_at.isdigit():
            ...
            offer.date_posted = datetime.datetime.utcfromtimestamp(
                int(created_at)
            ).strftime("%Y-%m-%d")
        elif created_at:
        ...
        # Intersecar con nuestras keywords para asegurar que son de TI.
        # Para Arbeitnow, solo confiamos en los extraídos que coinciden con el whitelist.
        valid_skills = [
            s for s in extracted_skills if s in GetOnBoardParser.HARD_SKILLS_KEYWORDS
        ]
